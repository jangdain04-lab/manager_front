...
function buildZoneRiskAlert(zone: {
  ...
  return {
    ...
    timestamp: new Date(),
    message: `${zone.name}(${zone.camera}) ${isDanger ? '위험' : '주의'} 단계가 감지되었습니다. 현재 감지 인원은 ${zone.peopleCount}명입니다.`,
    read: false,
...
export function Notifications() {
  ...
  const readAlertIds = useRef(new Set<string>());
  const pinnedAlertIds = useRef(new Set<string>());
  const deletedAlertIds = useRef(new Set<string>());
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      try {
        ...
        const alertItems = liveZones
          ...
          .filter((alert) => !deletedAlertIds.current.has(alert.id))
          .map((alert) => ({
            ...alert,
            read: readAlertIds.current.has(alert.id) || alert.read,
            pinned: pinnedAlertIds.current.has(alert.id) || alert.pinned,
          }));
    ...
    loadData();
    const timer = setInterval(loadData, DEMO_REFRESH_INTERVAL_MS);
  ...
  const togglePin = (id: string) => {
    ...
    if (nextPinned) {
      pinnedAlertIds.current.add(id);
    } else {
      pinnedAlertIds.current.delete(id);
    }
  ...
  const deleteAlert = (id: string) => {
    deletedAlertIds.current.add(id);
