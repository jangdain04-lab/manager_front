...
import { createVisitorReport, DEMO_REFRESH_INTERVAL_MS, fetchPublicLiveZones, fetchPublicMapUrl, resolveBackendUrl } from '../services/api';
...
export function HeatmapView() {
  ...
  useEffect(() => {
    ...
    const id = setInterval(loadLiveZones, DEMO_REFRESH_INTERVAL_MS);
...
function DensityCard({
  ...
  return (
    ...
    >
      ...
      <Text style={[styles.cameraLabel, { fontSize: 11 * scale }]}>
        {zone.camera}
      </Text>
...
function ReportModal({
  ...
  const [selectedPlace, setSelectedPlace] = useState('공대 정류장');
