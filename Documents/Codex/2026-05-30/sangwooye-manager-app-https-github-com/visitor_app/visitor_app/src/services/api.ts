...
const CAMERAS = [
  { id: '1', camera: 'cam 1', name: '도서관 앞', gridPos: { x: 29, y: 42 }, peopleOffset: 18 },
  { id: '2', camera: 'cam 2', name: '학생회관 앞', gridPos: { x: 74, y: 42 }, peopleOffset: 14 },
  { id: '3', camera: 'cam 3', name: '후생관 앞', gridPos: { x: 35, y: 72 }, peopleOffset: 16 },
  { id: '4', camera: 'cam 4', name: '공대 정류장', gridPos: { x: 78, y: 72 }, peopleOffset: 12 },
] as const;
...
const DEMO_FESTIVAL_INFO: FestivalInfo = {
  ...
  description: [
    '이 앱은 시연용 데모 데이터로 1분 단위 밀집도 변화를 반복 재생합니다.',
...
function getDemoRow(): DemoDensityRow {
  const index = Math.floor(Date.now() / 1000) % DEMO_DENSITY_ROWS.length;
...
function toCrowdZone(cameraIndex: number, rawDensity: number): CrowdZone {
  ...
  return {
    ...
    peopleCount: Math.max(0, rawDensity * 5 + camera.peopleOffset),
...
function buildZoneAlert(zone: CrowdZone): Alert | null {
  ...
  return {
    id: `demo-alert-${getDemoSecond()}-${zone.id}`,
    zoneId: zone.id,
    zoneName: zone.name,
    riskLevel: isDanger ? 'danger' : 'warning',
    timestamp: new Date(Date.now() - Number(zone.id) * 60 * 1000),
    message: `manager 앱에서 ${zone.name} 구역 ${isDanger ? '위험' : '주의'} 알림을 전송했습니다. 우회 동선을 이용해 주세요.`,
    read: false,
    pinned: isDanger,
